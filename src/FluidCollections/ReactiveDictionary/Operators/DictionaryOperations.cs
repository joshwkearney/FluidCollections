...
namespace FluidCollections {
    public static partial class ReactiveDictionaryExtensions {
        ...
        public static IReactiveDictionary<TKey, TValue> Except<TKey, TValue>(this IReactiveDictionary<TKey, TValue> dict, IReactiveSet<TKey> keys) {
            ...
            var obs2 = keys.AsObservable().Select(changes => changes.Select(change => {
                if (change.ChangeReason == ReactiveSetChangeReason.Add) {
                    return new ReactiveDictionaryChange<TKey, TValue>(change.Value, default, ReactiveDictionaryChangeReason.Remove);
                }
                else if (dict.TryGetValue(change.Value, out var value)) {
                    return new ReactiveDictionaryChange<TKey, TValue>(change.Value, value, ReactiveDictionaryChangeReason.AddOrUpdate);
                }
